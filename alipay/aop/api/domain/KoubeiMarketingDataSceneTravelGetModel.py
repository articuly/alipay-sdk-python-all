...
from alipay.aop.api.domain.TravelScene import TravelScene


class KoubeiMarketingDataSceneTravelGetModel(object):
    ...
    def biz_info(self, value):
        if isinstance(value, TravelScene):
            self._biz_info = value
        else:
            self._biz_info = TravelScene.from_alipay_dict(value)
    @property
    def ext_info(self):
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = KoubeiMarketingDataSceneTravelGetModel()
        if 'biz_info' in d:
