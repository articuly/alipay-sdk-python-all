...
from alipay.aop.api.domain.FengdieSitesListModel import FengdieSitesListModel
from alipay.aop.api.domain.FengdieListPaginator import FengdieListPaginator


class FengdieSitesListRespModel(object):
    ...
    def list(self, value):
        if isinstance(value, list):
            self._list = list()
            for i in value:
                if isinstance(i, FengdieSitesListModel):
                    self._list.append(i)
                else:
                    self._list.append(FengdieSitesListModel.from_alipay_dict(i))
    @property
    def paginator(self):
