...
from alipay.aop.api.domain.FengdieSitesPageModel import FengdieSitesPageModel


class FengdieSitesQueryRespModel(object):
    ...
    def pages(self, value):
        if isinstance(value, list):
            self._pages = list()
            for i in value:
                if isinstance(i, FengdieSitesPageModel):
                    self._pages.append(i)
                else:
                    self._pages.append(FengdieSitesPageModel.from_alipay_dict(i))
    @property
    def status(self):
        return self._status
