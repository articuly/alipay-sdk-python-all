...
class AlipayMdataTagGetRequest(object):

    def __init__(self, biz_model=None):
        self._biz_model = biz_model
        self._required_tags = None
        self._user_id = None
        self._version = "1.0"
    ...
    def required_tags(self, value):
        if isinstance(value, list):
            self._required_tags = list()
            for i in value:
                self._required_tags.append(i)
    @property
    def user_id(self):
        ...
        if self.required_tags:
            if isinstance(self.required_tags, list):
                for i in range(0, len(self.required_tags)):
                    element = self.required_tags[i]
                    if hasattr(element, 'to_alipay_dict'):
                        self.required_tags[i] = element.to_alipay_dict()
