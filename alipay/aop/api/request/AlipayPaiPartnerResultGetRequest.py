...
class AlipayPaiPartnerResultGetRequest(object):
    ...
    def get_params(self):
        params = dict()
        params[P_METHOD] = 'alipay.pai.partner.result.get'
        ...
        if self.partner_user_id:
            if hasattr(self.partner_user_id, 'to_alipay_dict'):
                params['partner_user_id'] = json.dumps(obj=self.partner_user_id.to_alipay_dict(), use_decimal=True, ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
