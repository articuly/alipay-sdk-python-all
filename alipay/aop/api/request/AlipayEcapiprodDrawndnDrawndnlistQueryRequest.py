...
class AlipayEcapiprodDrawndnDrawndnlistQueryRequest(object):
    ...
    def get_params(self):
        params = dict()
        params[P_METHOD] = 'alipay.ecapiprod.drawndn.drawndnlist.query'
        params[P_VERSION] = self.version
        if self.biz_model:
            params[P_BIZ_CONTENT] = json.dumps(obj=self.biz_model.to_alipay_dict(), use_decimal=True, ensure_ascii=False, sort_keys=True, separators=(',', ':'))
        if self.credit_no:
            if hasattr(self.credit_no, 'to_alipay_dict'):
                params['credit_no'] = json.dumps(obj=self.credit_no.to_alipay_dict(), use_decimal=True, ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
